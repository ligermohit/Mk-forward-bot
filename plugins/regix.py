...
async def pub_(bot, message):
    ...
    # Import plan checking function
    from plugins.plans import check_user_limit
    
    # Check user's plan limits before proceeding
    can_forward, user_plan = await check_user_limit(user)
    
    # If user has reached their limit, show them the limit reached message
    if not can_forward:
        await message.answer("Daily limit reached! Upgrade to premium plan for more forwarding.", show_alert=True)
        buttons = [[
            InlineKeyboardButton('💎 Contact Bot Owner', url=f'https://t.me/{(await bot.get_me()).username}?start=premium')
        ]]
        await message.message.edit_text(
            text=Script.PLAN_LIMIT_REACHED.format(user_plan["daily_limit"]),
    ...
    sts.add(time=True)
    # Use configured speed settings from config
    sleep = temp.BOT_FORWARD_SPEED if _bot['is_bot'] else temp.USERBOT_FORWARD_SPEED
    await msg_edit(m, f"<code>processing... (speed: {sleep}s delay)</code>") 
    temp.IS_FRWD_CHAT.append(i.TO)
    
    # Log forwarding start to channel
    try:
        from plugins.logger import log_to_channel
        from config import Config
        
        # Get user name
        user_info = await db.col.find_one({'id': int(user)})
        name = user_info.get('name', 'Unknown') if user_info else 'Unknown'
        
        # Get channel source and destination details
        from_chat_id = i.FROM
        to_chat_id = i.TO
        start_id = i.START
        end_id = i.END
        
        # Log forwarding start
        await log_to_channel(
            ...
            details={"forward_details": {
                ...
                "start_id": start_id,
                "end_id": end_id,
                "bot_used": _bot['username']
            }},
            user_info={"id": user, "name": name}
        )
    except Exception as e:
        print(f"Error logging forwarding start: {e}")
    ...
    if locked:
        try:
          ...
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=sts.get("skip"), filters=filter, max_size=max_size, start_id=sts.get("start_id"), end_id=sts.get("end_id")):
                ...
                if forward_tag:
                   ...
                   if ( notcompleted >= 100  # Reduced batch size to avoid FloodWait 
                        or completed <= 100): 
                      await forward(user, client, MSG, m, sts, protect)
                      sts.add('total_files', notcompleted)
                      await asyncio.sleep(temp.BOT_FORWARD_SPEED/2)  # Reduced sleep time for faster forwarding
                      MSG = []
                else:
        ...
        # Import the update_user_forwarded_count function
        from plugins.plans import update_user_forwarded_count
        
        # Update the user's plan with the number of files forwarded
        total_files_forwarded = sts.get('total_files')
        await update_user_forwarded_count(user, total_files_forwarded)
        
        # Log forwarding completion to channel
        try:
            from plugins.logger import log_to_channel
            from config import Config
            import time
            ...
            start_time = sts.get('start', time.time())
            total_time_sec = time.time() - start_time
            total_time = TimeFormatter(milliseconds=int(total_time_sec * 1000))
            
            # Log forwarding completion
            await log_to_channel(
                client,
                "forwarding_complete",
                details={"forward_details": {
                    "from_chat": from_chat_id,
                    "to_chat": to_chat_id,
                    "success": sts.get('total_files', 0),
                    "duplicate": sts.get('duplicate', 0),
                    "failed": sts.get('deleted', 0) + sts.get('filtered', 0),
                    "total_time": total_time
                }},
                user_info={"id": user, "name": name}
            )
        except Exception as e:
            print(f"Error logging forwarding completion: {e}")
...
async def copy(user, bot, msg, m, sts):
   try:                               
     ...
     # Add 15 extra seconds to the wait time to be safe
     wait_time = e.value + 15
     # Use the configurable bot speed for messages
     delay_time = temp.BOT_FORWARD_SPEED * 3  # Triple delay for retry
     await edit(user, m, 'ᴘʀᴏɢʀᴇssɪɴɢ', delay_time, sts)
...
async def forward(user, bot, msg, m, sts, protect):
   try:                             
     ...
     # Add 10 extra seconds to the wait time to be safe
     wait_time = e.value + 10
     # Use the configurable bot speed for messages
     delay_time = temp.BOT_FORWARD_SPEED * 2  # Double delay for retry
     await edit(user, m, 'ᴘʀᴏɢʀᴇssɪɴɢ', delay_time, sts)
     await asyncio.sleep(delay_time)
     await edit(user, m, 'ᴘʀᴏɢʀᴇssɪɴɢ', 5, sts)
     # Try again with user parameter to avoid errors
...
async def msg_edit(msg, text, button=None, wait=None):
    try:
        ...
        if wait:
           # Add extra buffer time plus dynamic delay based on current bot speed 
           wait_time = e.value + temp.BOT_FORWARD_SPEED
...
async def edit(user, msg, title, status, sts):
   ...
   # Use the stored start_id and end_id
   start_id = getattr(i, 'start_id', i.skip)
   last_id = getattr(i, 'end_id', i.limit)
   
   # Extract chat_id from FROM to create the links
   # Telegram requires removing the -100 prefix for the URL
   chat_id_str = str(i.FROM)
   if chat_id_str.startswith('-100'):
      chat_id_for_url = chat_id_str[4:]
   else:
      chat_id_for_url = chat_id_str.lstrip('-')
   
   # Format the text with the updated information including start_id and last_id as links
   text = TEXT.format(
      chat_id_for_url, start_id, start_id,  # First set for start_id link
      chat_id_for_url, last_id, last_id,    # Second set for last_id link
      i.fetched, i.total_files, i.duplicate, i.deleted, 
      i.skip, i.filtered, status, percentage, title
   )
   
   await update_forward(user_id=user, last_id=last_id, start_time=i.start, limit=i.limit, chat_id=i.FROM, toid=i.TO, forward_id=None, msg_id=msg.id, fetched=i.fetched, deleted=i.deleted, total=i.total_files, duplicate=i.duplicate, skip=i.skip, filterd=i.filtered)
   now = time.time()
   ...
   if status in ["cancelled", "completed"]:
      button.append([InlineKeyboardButton('• ᴄᴏᴍᴘʟᴇᴛᴇᴅ ​•', url='https://t.me/+wD-yNhKhYd5hNzA1')])
   else:
...
async def status_msg(bot, msg):
    ...
    if not sts.verify():
       ...
       start_id = sts.get('start_id', 1)
       end_id = sts.get('end_id', limit)
    ...
    return await msg.answer(PROGRESS.format(percentage, start_id, end_id, fetched, forwarded, remaining, status, time_to_comple, uptime), show_alert=True)
...
async def restart_pending_forwads(bot, user):
    ...
    # Speed optimized for faster forwarding (reduced from 10s to 1.0s for userbot mode)
    sleep = temp.BOT_FORWARD_SPEED if _bot['is_bot'] else temp.USERBOT_FORWARD_SPEED
    #await msg_edit(m, "<code>processing...</code>") 
    temp.IS_FRWD_CHAT.append(i.TO)
    temp.lock[user] = locked = True
    ...
    if locked:
        try:
          ...
          pling=0
          await edit(user, m, 'ᴘʀᴏɢʀᴇssɪɴɢ', 5, sts)
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=skiping, filters=filter, max_size=max_size, start_id=sts.get("start_id"), end_id=sts.get("end_id")):
                if await is_cancelled(client, user, m, sts):
                    if user_have_db:
                       await user_db.drop_all()
                       await user_db.close()
...
async def store_vars(user_id):
    ...
    print(fetch)
    # Get start_id and end_id if available, otherwise use defaults
    start_id = settings.get('start_id', 1)
    end_id = settings.get('end_id', settings['limit'])
    STS(id=forward_id).store(settings['chat_id'], settings['toid'], settings['skip'], settings['limit'], start_id=start_id, end_id=end_id)
    return forward_id
...
async def update_forward(user_id, chat_id, start_time, toid, last_id, limit, forward_id, msg_id, fetched, total, duplicate, deleted, skip, filterd):
    # Get the STS object to get the start_id and end_id values
    sts = STS(user_id)
    if sts.verify():
        start_id = sts.get('start_id')
        end_id = sts.get('end_id')
    else:
