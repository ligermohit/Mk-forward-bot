...
COMPLETED_BTN = InlineKeyboardMarkup(
  [[
    InlineKeyboardButton('💟 sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ 💟', url='https://t.me/iCrunchKorn Bots')
  ],[
    InlineKeyboardButton('💠 ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ 💠', url='https://t.me/itz_Thanos')
  ]]
)
...
async def unequify(client, message):
   ...
   try:
     ...
     # Use a set for faster lookups and memory efficiency
     MESSAGES = set()
     DUPLICATE = []
     batch_size = 200  # Increased batch size for faster processing
     update_interval = 500  # Update status less frequently for better performance
     ...
     async for message in bot.search_messages(chat_id=chat_id, filter=enums.MessagesFilter.DOCUMENT):
        ...
        # Check if it's a duplicate using set (faster than list)
        if file_id in MESSAGES:
           DUPLICATE.append(message.id)
        else:
           MESSAGES.add(file_id)  # Use add() for sets instead of append()
        ...
        # Delete in larger batches for better efficiency
        if len(DUPLICATE) >= batch_size:
