...
async def settings(client, message):
    # Import the channel check function
    from plugins.commands import check_user_subscription
    
    # Check if user is subscribed to the force channel
    is_subscribed = await check_user_subscription(client, message.from_user.id, message)
    
    if is_subscribed:
        # If user is subscribed, show settings
        await message.reply_text(
          "<b>Hᴇʀᴇ Is Tʜᴇ Sᴇᴛᴛɪɴɢs Pᴀɴᴇʟ⚙\n\nᴄʜᴀɴɢᴇ ʏᴏᴜʀ sᴇᴛᴛɪɴɢs ᴀs ʏᴏᴜʀ ᴡɪsʜ 👇</b>",
          reply_markup=main_buttons()
        )
    # If not subscribed, the check_user_subscription function will handle the subscription message
  ...
  if type=="main":
     ...
     try:
         ...
         if _bot is not None:
            try:
                ...
                print(f"Error getting bot name for user {user_id}: {e}")
     ...
     try:
         ...
         if usr_bot is not None:
            try:
                ...
                print(f"Error getting userbot name for user {user_id}: {e}")
         ...
         if channels:
             for channel in channels:
                try:
                    ...
                    chat_id = channel.get('chat_id', '0')
                    buttons.append([InlineKeyboardButton(f"{title}",
                                 callback_data=f"settings#editchannels_{chat_id}")])
                except (KeyError, TypeError) as e:
                    print(f"Error processing channel for user {user_id}: {e}")
     except Exception as e:
         print(f"Error getting channels for user {user_id}: {e}")
     
     # Always include add channel button regardless of errors
     ...
     try:
         ...
         if bot and 'is_bot' in bot:
             ...
             await query.message.edit_text(
                TEXT.format(name, bot_id, username),
                reply_markup=InlineKeyboardMarkup(buttons))
         else:
             # Bot data not found or invalid, return to bots menu
             buttons = [[InlineKeyboardButton('back', callback_data="settings#bots")]]
             await query.message.edit_text(
                "<b>Error: Bot data not found or incomplete.</b>",
                reply_markup=InlineKeyboardMarkup(buttons))
     except Exception as e:
         print(f"Error displaying bot details for user {user_id}: {e}")
         buttons = [[InlineKeyboardButton('back', callback_data="settings#bots")]]
         await query.message.edit_text(
            "<b>Error retrieving bot details. Please try again later.</b>",
            reply_markup=InlineKeyboardMarkup(buttons))
     
  elif type=="edituserbot": 
     try:
         bot = await db.get_userbot(user_id)
         if bot and all(key in bot for key in ['name', 'id', 'username']):
             ...
             # Userbot data not found or invalid, return to bots menu
             buttons = [[InlineKeyboardButton('back', callback_data="settings#bots")]]
             await query.message.edit_text(
                "<b>Error: Userbot data not found or incomplete.</b>",
                reply_markup=InlineKeyboardMarkup(buttons))
     except Exception as e:
         print(f"Error displaying userbot details for user {user_id}: {e}")
         buttons = [[InlineKeyboardButton('back', callback_data="settings#bots")]]
         await query.message.edit_text(
            "<b>Error retrieving userbot details. Please try again later.</b>",
            reply_markup=InlineKeyboardMarkup(buttons))
     ...
     try:
         ...
         if chat and all(key in chat for key in ['title', 'chat_id', 'username']):
             ...
             # Channel data not found or invalid
             buttons = [[InlineKeyboardButton('back', callback_data="settings#channels")]]
             await query.message.edit_text(
                "<b>Error: Channel data not found or incomplete.</b>",
                reply_markup=InlineKeyboardMarkup(buttons))
     except Exception as e:
         print(f"Error displaying channel details for user {user_id}, channel {chat_id}: {e}")
         buttons = [[InlineKeyboardButton('back', callback_data="settings#channels")]]
         await query.message.edit_text(
            "<b>Error retrieving channel details. Please try again later.</b>",
            reply_markup=InlineKeyboardMarkup(buttons))
